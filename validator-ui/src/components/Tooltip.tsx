...
export function useTooltip<T extends HTMLElement = HTMLElement>(
  options: { position?: TooltipPosition; delay?: number } = {}
): UseTooltipReturn<T> {
  ...
  return {
    triggerRef,
    triggerProps: { onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave },
    tooltipProps: { isVisible, coords, position },
  }
}
...
export function TooltipPortal({ isVisible, coords, position, content, className = '' }: {
  isVisible: boolean; coords: TooltipCoords; position: TooltipPosition; content: ReactNode; className?: string
}) {
  return createPortal(
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          style={{
            ...
            transform: getTransform(position), transformOrigin: getTransformOrigin(position), zIndex: 9999,
          }}
          className={`bg-hud-bg-panel border border-hud-line px-3 py-2 text-xs text-hud-text max-w-xs pointer-events-none ${className}`}
        >
          {content}
        </motion.div>
      )}
...
export function Tooltip({ children, content, position = 'top', delay = 200, className = '' }: {
  children: ReactNode; content: ReactNode; position?: TooltipPosition; delay?: number; className?: string
}) {
  const { triggerRef, triggerProps, tooltipProps } = useTooltip<HTMLDivElement>({ position, delay })
  return (
    <>
      <div ref={triggerRef} {...triggerProps} className="inline-block">{children}</div>
      <TooltipPortal {...tooltipProps} content={content} className={className} />
    </>
  )
}

export function TooltipContent({ title, items, description }: {
  title?: string; items?: Array<{ label: string; value: string | number; color?: string }>; description?: string
}) {
  return (
    <div className="space-y-2">
      {title && <div className="hud-label text-hud-primary border-b border-hud-line/50 pb-1">{title}</div>}
      ...
      {description && <p className="text-hud-text-dim text-[10px] leading-tight">{description}</p>}
    </div>
  )
}
